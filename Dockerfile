...
RUN PACKER_VER=($(curl -s https://releases.hashicorp.com/packer/ | grep '\/[0-9][^a-zA-Z]*[0-9]\/' | cut -d / -f 3 | head -n 1)) && \
    PACKER_ZIP=https://releases.hashicorp.com/packer/$PACKER_VER/packer_$PACKER_VER_linux_amd64.zip && \
    curl -sSLo /tmp/packer.zip $PACKER_ZIP && \
